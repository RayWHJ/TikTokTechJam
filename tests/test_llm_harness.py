...
from llm_calls import dedup as dedup_mod
...
def test_diagnose_retries_then_succeeds(monkeypatch):
    ...
    monkeypatch.setattr(diagnose_mod, "call_model_text", lambda system, p, model=None: call_fn(p))
...
def test_ground_in_literature_caches_repeat_lookups(monkeypatch, isolated_literature_cache):
    ...
    monkeypatch.setattr(literature_mod, "call_model_with_search",
                         lambda system, p, model=None: call_fn(p))
...
def test_generate_hypothesis_high_confidence_requests_one(monkeypatch):
    valid_one = json.dumps([_fake_hypothesis(1)])
    call_fn = _make_sequenced_call_fn([valid_one])
    monkeypatch.setattr(hypothesis_mod, "call_model_text", lambda system, p, model=None: call_fn(p))

    result = hypothesis_mod.generate_hypothesis(HIGH_CONFIDENCE_DIAGNOSIS, FAKE_EVIDENCE_CARD)

    assert len(result) == 1
    assert "Produce hypothesis" in call_fn.calls["prompts"][0]  # singular phrasing used for count=1


def test_generate_hypothesis_low_confidence_requests_three(monkeypatch):
    valid_three = json.dumps([_fake_hypothesis(i) for i in range(3)])
    call_fn = _make_sequenced_call_fn([valid_three])
    monkeypatch.setattr(hypothesis_mod, "call_model_text", lambda system, p, model=None: call_fn(p))
...
def test_generate_hypothesis_plateau_text_also_requests_three(monkeypatch):
    valid_three = json.dumps([_fake_hypothesis(i) for i in range(3)])
    call_fn = _make_sequenced_call_fn([valid_three])
    monkeypatch.setattr(hypothesis_mod, "call_model_text", lambda system, p, model=None: call_fn(p))
...
def test_generate_hypothesis_rejects_flat_absolute_threshold(monkeypatch):
    bad_hypothesis = _fake_hypothesis(1)
    bad_hypothesis["success_criterion_paired"] = "nDCG@5 reaches 0.35"  # flat threshold, no delta/tier language
    bad_response = json.dumps([bad_hypothesis])
    good_response = json.dumps([_fake_hypothesis(1)])
...
def test_generate_hypothesis_wrong_array_length_triggers_retry(monkeypatch):
    wrong_length = json.dumps([_fake_hypothesis(1), _fake_hypothesis(2)])  # asked for 1, got 2
    correct_length = json.dumps([_fake_hypothesis(1)])
...
def test_audit_detects_violation(monkeypatch):
    ...
    monkeypatch.setattr(audit_mod, "call_model_text", lambda system, p, model=None: call_fn(p))
...
def _forbid_model_call(system, p, model=None, max_tokens=None):
    raise AssertionError("model should not have been called for this case")


def test_dedup_exact_match_no_model_call(monkeypatch):
    monkeypatch.setattr(dedup_mod, "call_model_text", _forbid_model_call)

    candidate = ("din_attention", "user_history_len_20", "loss=bce")
    memory = [("din_attention", "user_history_len_20", "loss=bce")]

    assert dedup_mod.dedup_fingerprint_match(candidate, memory) is True


def test_dedup_clearly_distinct_no_model_call(monkeypatch):
    monkeypatch.setattr(dedup_mod, "call_model_text", _forbid_model_call)

    candidate = ("din_attention", "user_history_len_20", "loss=bce")
    memory = [("esmm_multitask", "shared_bottom", "loss=pairwise")]

    assert dedup_mod.dedup_fingerprint_match(candidate, memory) is False


def test_dedup_ambiguous_case_escalates_to_model(monkeypatch):
    dup_response = json.dumps({"duplicate": True, "reasoning": "same architecture, only a minor hparam differs"})
    call_fn = _make_sequenced_call_fn([dup_response])
    monkeypatch.setattr(dedup_mod, "call_model_text",
                         lambda system, p, model=None, max_tokens=None: call_fn(p))

    candidate = ("din_attention", "user_history_len_20", "loss=bce")
    memory = [("din_attention", "user_history_len_50", "loss=bce")]  # differs in exactly 1 position

    result = dedup_mod.dedup_fingerprint_match(candidate, memory)

    assert call_fn.calls["count"] == 1
    assert result is True


def test_dedup_ambiguous_case_model_says_not_duplicate(monkeypatch):
    not_dup_response = json.dumps({"duplicate": False, "reasoning": "history length is a meaningfully different design choice"})
    call_fn = _make_sequenced_call_fn([not_dup_response])
    monkeypatch.setattr(dedup_mod, "call_model_text",
                         lambda system, p, model=None, max_tokens=None: call_fn(p))

    candidate = ("din_attention", "user_history_len_20", "loss=bce")
    memory = [("din_attention", "user_history_len_50", "loss=bce")]
