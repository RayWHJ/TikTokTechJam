...
from llm_calls import diagnose, ground_in_literature, generate_hypothesis, audit
import codegen
# create shim so `llm.diagnose(...)` etc. still work:
class _LLM:
...
def run(max_iters: int = 50, wallclock_cap_s: int = 6 * 3600, verbose: bool = True):
    ...
    for it in range(1, max_iters + 1):
        print(f"[iter {it}] open_nodes={len(open_nodes)} global_best={global_best:.4f}")   # <— ADD
        ...
        if global_should_stop(open_nodes, max_iters - it + 1, global_best):
            print(f"[iter {it}] global_should_stop TRIGGERED")                              # <— ADD
            break
        ...
        print(f"[iter {it}] hypotheses={len(hypotheses)} candidates={len(candidates)}")     # <— ADD
