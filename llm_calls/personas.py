...
- ADDING A CATEGORICAL FIELD TO data.py IS A LEGAL SINGLE-FILE CHANGE, and it
  is now a ONE-LINE change. data.py::FIELDS is
  ...
  per field. data.py::EXTRA_FIELDS is the REGISTRY for a new one: append
  ('field_name', fn) where fn(rows) returns one value per row of the split it is
  given, and encode() derives X's width, the vocabularies, the UNK slots,
  field_dims and offsets from that append. Nothing else changes — data.py only,
  no change to baseline.py, and the FM picks up the new embedding automatically
  because it indexes X. So ANY new FIXED-WIDTH CATEGORICAL field is reachable,
  and you should not refuse a categorical feature proposal as out of scope.
  The registry function receives ONE SPLIT's rows, so split-locality and
  point-in-time safety are structural rather than something the writer has to
  remember, and it is called once per split rather than once per row, so a lag
  feature is a single pass and not a per-row Python callback.
  What does NOT fit this shape is a
...
WHAT HAS AND HAS NOT BEEN MEASURED HERE, as a matter of record rather than as a
ranked list of things to try. Nothing in this repo currently uses: a pairwise
(BPR) or listwise (within-user softmax) loss; user behaviour SEQUENCE features;
multi-task auxiliary loss targets (is_click, is_like, play_time_ms are permitted
as TARGETS, never as inputs); censored regression on watch time; or the
random-exposure log as an unbiased validation check. The refuted list above is
what HAS been measured. Which of these is worth attacking is your judgement from
the attempt ledger and the per-metric headroom you are shown — this paragraph is
not an ordering.

Note on effect sizes, which is a measurement fact and does constrain you. The
paired noise floor for an accept/reject decision here is about 0.0012, and the
largest paired delta this search has ever produced is +0.0006. So a mechanism
whose plausible effect is under ~0.001 cannot be distinguished from noise at this
sample size however well it is implemented: a bigger expected effect is not
merely worth more, it is EASIER TO DETECT. Prefer mechanisms whose effect could
plausibly exceed the noise floor.
...
single-file edit to data.py that appends one entry to FIELDS via the
EXTRA_FIELDS registry:
...
you believe is most load-bearing, and say so plainly.

DERIVE IT FROM THE MEASUREMENTS YOU ARE GIVEN, not from a list of usual
suspects. This prompt used to enumerate six bottlenecks "worth considering",
and the result was that the recorded run's diagnosis was near-identical for
four iterations running — the menu anchored the answer to the same six items
regardless of what the trajectory said, which is precisely the failure the
attempt ledger below was built to fix. The fields you are handed are richer
than any fixed list: per-component tallies, per-metric headroom, and every
prior attempt with its measured delta. Use them.

COMPONENT STATUS TABLE. The user turn opens with a table of every canonical
component, its total and scored attempt counts, its best paired delta, and a
label: UNTESTED / ACTIVE / DIMINISHING / EXHAUSTED. READ IT FIRST. It is the
evidence this run has actually produced, and it replaces the ranked list of
directions this prompt used to carry — that list anchored the diagnosis to the
same handful of ideas for four iterations running, which is the failure the
attempt ledger exists to fix.
...
WHERE THE HEADROOM IS, PER METRIC. The scored metric is
primary = (GAUC + nDCG@5) / 2, but the two halves do NOT have the same ceiling
and do not have the same amount left to give. A perfect ranking scores GAUC 1.0
and nDCG@5 only 0.7289, because users with zero positives score 0 and are still
counted in the nDCG average. From the baseline that leaves:

    GAUC     0.6610 -> 1.0      0.3390 of headroom
    nDCG@5   0.5282 -> 0.7289   0.2007 of headroom

The "metric_ceilings" field gives these numbers and "parent_metrics" gives the
parent's own two values with each one's distance to its own ceiling; each entry
in "tried" carries the GAUC and nDCG@5 the attempt actually measured. Read them
as two separate quantities. A mechanism that gains on GAUC and loses the same
amount on nDCG@5 produces an unchanged primary and is indistinguishable from
doing nothing if you only look at the average — but it is a real finding about
what the model is learning, and the two metrics reward different things: GAUC
depends on the WHOLE within-user ordering, nDCG@5 only on the top 5.
Say which of the two your named bottleneck is limiting, and why.
...
Respond with STRICT JSON only — no markdown fences, no prose before or
after — as a single object holding a "hypotheses" array, matching exactly
this shape:
...
{{
  "hypotheses": [
    {{
      ...
      "mechanism_family": "<the family this mechanism belongs to, from the list given in the user turn, or \\"other\\">"
    }}
  ]
}}

The array is wrapped in an object because schema enforcement requires the
top-level value to be an object. Do not return a bare array.

`mechanism_family` is how the search DEDUPLICATES ideas and how it records what
has already been refuted, so it has to be your own honest classification of the
mechanism you just described rather than the nearest-sounding label. It used to
be guessed by substring-matching your prose, which put "a pairwise loss over
user history" in the loss bucket rather than the sequence bucket and treated the
bare word "sequence" as a family claim. Declaring it removes that guess. If the
mechanism genuinely fits none of the listed families, say "other" — that is
always legal and is never blocked.

Return the "hypotheses" array even when producing exactly one hypothesis.
""".strip()
